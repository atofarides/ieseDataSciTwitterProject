...
pol <- tweetsSentimentsAfinn %>%
      group_by(airline,tweetNumber) %>%
      summarise(polarity = sum(score)) %>%
      ungroup()

ggplot(pol, aes(x=polarity)) + geom_histogram(binwidth = 1, color="black",fill="white") + 
  facet_wrap(facets = "airline", scales = "free") + 
  ggsave(file.path("~", "Github","ieseDataSciTwitterProject", "airlinePolarityAfinn.pdf", fsep = "/"))
...
# Negative: None explicitly

# Create polarity thresholds to rank airlines. Polarity threshold levels were decided
